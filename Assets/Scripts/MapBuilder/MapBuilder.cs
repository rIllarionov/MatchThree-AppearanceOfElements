...
public class MapBuilder : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _halfMapSize = new Vector2((_countItemsColumn / 2), _countItemsRow / 2);
    ...
    private void InitializeMap()
    {
        ...
        for (int i = 0; i < _countItemsRow; i++)
        {
            for (int j = 0; j < _countItemsColumn; j++)
            {
                ...
                _tileMap[i, j].transform.localPosition =
                    (new Vector2(j,i ) - _halfMapSize) * _cellOffset;
            }
        }
    }
    ...
    {
        //предпологаем что элемент нам сразу подходит В процессе меняем на флаг на фолс если элемент не подходит
